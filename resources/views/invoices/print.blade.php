...
<div class="invoice-wrapper">
            ...
            <td style="width: 70%;">
                <div class="store-info">
                    ...
                    <div class="store-details">
                        ...
                        <p>السجل التجاري: {{ optional(optional($invoice->sale)->store)->commercial_registration ?? 'غير مسجل' }}</p>
                        <p>الهاتف: {{ optional(optional($invoice->sale)->store)->phone ?? 'غير مسجل' }}</p>
                        <p>الرقم الضريبي: <strong>{{ optional(optional($invoice->sale)->store)->tax_number ?? '3000XXXXXXXXXXX' }}</strong></p>
                    </div>
                </div>
            </td>
            <td style="width: 30%; text-align: left;">
                {{-- الباركود بصيغة SVG لضمان العمل بدون Imagick --}}
                {!! QrCode::size(85)->generate($invoice->zatca_qr_code ?? ($invoice->invoice_number ?? 'invoice')) !!}
    ...
    <div class="bank-info">
        {{ optional(optional($invoice->sale)->store)->bank_accounts_info ?? 'مصرف الراجحي: SA0000000000000000000000 | البنك الأهلي: SA0000000000000000000000' }}
    </div>

    <div style="margin-top: 15px; text-align: center; font-size: 10px; color: #94a3b8;">
        @php
            $paymentType = match (optional($invoice->sale)->sale_type) {
                'cash' => 'نقداً',
                'card' => 'شبكة',
                'mixed' => 'مختلط',
                'credit' => 'آجل',
        ...
        | المحاسب: {{ optional(optional($invoice->sale)->accountant)->name ?? optional(optional(optional($invoice->sale)->store)->user)->name ?? '—' }}
    </div>
</div>
